...
describe("Carpenter", () => {
  ...
  it("should generate a query with a WHERE and JOIN statement", () => {
    ...
    expect(sql.toString()).toEqual(
      "SELECT * FROM table1 INNER JOIN table2 ON table1.id = table2.table1_id WHERE id = 1"
    );
  });
  ...
  it("should generate a query in the proper order with multiple WHERE and JOIN statement in awakward orders", () => {
    ...
    expect(sql.toString()).toEqual(
      `SELECT * FROM table1 INNER JOIN table2 ON table1.id = table2.table1_id INNER JOIN table3 ON table2.id = table3.table2_id WHERE table2.name IN ('test') AND table3.name IN ('test')`
    );
  });
  ...
  it("should generate a query with multiple where statements that maintain the order as used", () => {
    ...
    expect(sql.toString()).toEqual(
      `SELECT * FROM table1 WHERE id = 1 AND id = 2 OR id = 3 AND id = 4`
    );
  });

  it("should not include OR if there is only one WHERE statement", () => {
    const sql = carpenter.from("table1").select("*").orWhere("id = 1");
  ...
  it("should generate an SQL query with raw statements at the end", () => {
    ...
    expect(sql.toString()).toEqual(
      "SELECT * FROM table1 WHERE id = 1 ORDER BY created_at"
    );
  });
  ...
  describe("joins", () => {
    ...
    test.each(cases)(
      "given method '%s' it should return SQL query that includes '%s'",
      (methodName, expected) => {
        ...
        expect(sql.toString()).toEqual(
          `SELECT * FROM table1 ${expected} table2 ON table1.id = table2.table1_id`
        );
      }
    );
  });
